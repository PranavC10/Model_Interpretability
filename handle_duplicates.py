...
# Load existing IDs from the output CSV to avoid duplicates
def load_written_ids():
    try:
        existing_data = pd.read_csv(output_csv)
        return set(existing_data['id'].astype(str).tolist())
    except FileNotFoundError:
        return set()  # Output file does not exist yet

# Initialize sets for processed pairs and written IDs
...
with open(jsonl_file, 'r') as f:
    for line in tqdm(f, desc="Processing news records"):
        ...
        news_id = str(news_record.get('id'))
        
        # Skip if this ID has already been written to the output CSV
        if news_id in written_ids:
            continue
        ...
        for _, customer_row in customer_df.iterrows():
            ...
            if check_name_in_body(customer_name, news_record['original_body']):
                ...
                # Add this ID to the set of written IDs to avoid duplicates
                written_ids.add(news_id)
